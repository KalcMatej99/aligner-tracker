...
MVP test entry requires a debug APK plus setup, tracking, corrections, explicit tray progression/completion, covered-time summaries, opted-in reminders and local backup/restore; build/lint/JVM checks and emulator smoke recorded. The owner has authorized a sole-tester Pixel pilot on 2026-09-09; [the session checklist](pixel-acceptance.md) covers all phone features and gathers real usability/device evidence together. It replaces five-person recruitment and does not wait for broad OEM, battery or watch coverage. A compiled APK alone does not close device acceptance issues.
...
## Owner Pixel acceptance — 2026-09-09

The owner is the only pilot tester. Complete [Pixel acceptance](pixel-acceptance.md): verify the installable phone APK and signer, protect any existing records, run the core journey, then every phone feature and coordinated lifecycle/accessibility case with synthetic data before normal personal use. Preparation and automated checks can proceed now; installation, actual interactions and disruptive phone checks take place with the owner tomorrow. Record PASS/FAIL/NOT RUN/BLOCKED per case; no physical result is prefilled. #18 and the #22 usability dependency stay open until that evidence is reviewed.

The owner's Garmin does not provide Wear OS acceptance. #20 paired/physical Wear remains open without removing it from v1 or adding Garmin integration. Wider OEM/API reminder coverage (#11), the full accessibility audit (#12), matched 24-hour battery observations (#17), and release signing/publication (#21) remain separate from this phone pilot. There is no five-person, multi-OEM or fixed-day obligation for starting the owner's session.
