...
class WearMathTest {
    ...
    private fun snapshot(date: String, hour: Int = 0): TrackerSnapshot {
        ...
        return TrackerSnapshot(
            ...
            listOf(WearEvent(1, start, true)),
        )
    }

    @Test
    fun `spring DST has 23 tracked hours`() {
    ...
    @Test
    fun `autumn DST has 25 tracked hours`() {
        val state = snapshot("2025-10-26")
        val summary =
            WearMath.summarize(
                state,
                LocalDate.parse("2025-10-26"),
                Instant.ofEpochMilli(at("2025-10-27", 0)),
            )
        assertEquals(25 * 3_600_000L, summary.wornMillis)
    }

    @Test
    fun `partial first day never invents earlier wear and clips now`() {
        val state = snapshot("2026-03-28", 12)
        val summary =
            WearMath.summarize(
                state,
                LocalDate.parse("2026-03-28"),
                Instant.ofEpochMilli(at("2026-03-28", 15)),
            )
    ...
    @Test
    fun `intervals spanning midnight split wearing and removed`() {
        val state =
            snapshot("2026-03-27", 23).let {
                it.copy(
                    events =
                        it.events +
                            WearEvent(2, at("2026-03-28", 1), false) +
                            WearEvent(3, at("2026-03-28", 3), true)
                )
            }
        val summary =
            WearMath.summarize(
                state,
                LocalDate.parse("2026-03-28"),
                Instant.ofEpochMilli(at("2026-03-28", 5)),
            )
    ...
    @Test
    fun `completion freezes totals including the last active interval`() {
        val state =
            snapshot("2026-03-28", 12).let {
        ...
        val summary =
            WearMath.summarize(
                state,
                LocalDate.parse("2026-03-28"),
                Instant.ofEpochMilli(at("2026-04-01", 0)),
            )
    ...
    @Test
    fun `future day and clock before initial event have zero coverage`() {
        val state = snapshot("2026-03-28", 12)
        assertEquals(
            0,
            WearMath.summarize(
        ...
        assertEquals(
            ...
            WearMath.summarize(
                    state,
                    LocalDate.parse("2026-03-28"),
                    Instant.ofEpochMilli(at("2026-03-28", 11)),
                )
                .trackedMillis,
        )
    }
    ...
    @Test
    fun `schedule uses calendar dates and current goal is consistent historically`() {
        val state =
            snapshot("2026-03-28").let { it.copy(plan = it.plan!!.copy(dailyGoalMinutes = 1320)) }
        ...
        assertEquals(
            ...
            WearMath.summarize(
                    state,
                    LocalDate.parse("2026-03-28"),
                    Instant.ofEpochMilli(at("2026-03-29", 0)),
                )
                .goalMinutes,
        )
    }
