...
object BackupCodec {
    ...
    @Serializable
    private data class Backup(val schemaVersion: Int, val plan: TreatmentPlan?, val events: List<WearEvent>)
    ...
    fun decode(json: String): TrackerSnapshot {
        require(json.length <= TrackerValidation.MAX_BACKUP_BYTES &&
        ...
        require(backup.schemaVersion == 1) { "Unsupported backup version. This app accepts schema 1." }
    ...
    fun csv(snapshot: TrackerSnapshot, now: Instant): String {
        ...
        val last = Instant.ofEpochMilli(minOf(now.toEpochMilli(), plan.completedAt ?: Long.MAX_VALUE))
            .atZone(zone).toLocalDate()
        return buildString {
            append(header)
            while (date <= last) {
                val summary = WearMath.summarize(snapshot, date, now)
                val dayMillis = date.plusDays(1).atStartOfDay(zone).toInstant().toEpochMilli() -
            ...
            when (source[i]) {
                ...
                '"' -> {
                    ...
                    if (frame?.expectsKey == true) {
                        val key = try { json.decodeFromString<String>(source.substring(start, i + 1)) }
