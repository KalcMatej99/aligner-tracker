...
object TrackerValidation {
    ...
    fun plan(plan: TreatmentPlan, now: Instant) {
        ...
        val zone = try { ZoneId.of(plan.zoneId) } catch (_: Exception) {
    ...
    fun snapshot(snapshot: TrackerSnapshot, now: Instant) {
        ...
        require(snapshot.events.first().at == plan.trackingStartedAt) { "Initial event must match tracking start." }
    ...
    private fun date(value: String): LocalDate {
        val parsed = try { LocalDate.parse(value) } catch (_: Exception) {
