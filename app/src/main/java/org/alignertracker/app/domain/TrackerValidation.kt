...
object TrackerValidation {
    ...
    fun plan(plan: TreatmentPlan, now: Instant) {
        ...
        require(plan.currentTray in 1..plan.totalTrays) { "Current tray must be within the treatment." }
        ...
        require(start <= current && current <= today) { "Tray start must be between treatment start and today." }
        require(plan.trackingStartedAt in 0..now.toEpochMilli()) { "Tracking start cannot be in the future." }
        ...
        require(plan.completed == (plan.completedAt != null)) { "Completed treatment needs a completion timestamp." }
        plan.completedAt?.let {
            require(it in plan.trackingStartedAt..now.toEpochMilli()) { "Completion must be within recorded treatment time." }
    ...
    fun snapshot(snapshot: TrackerSnapshot, now: Instant) {
        require(snapshot.events.size <= MAX_EVENTS) { "A backup can contain at most 50,000 events." }
    ...
    fun expandLegacy(snapshot: TrackerSnapshot): TrackerSnapshot {
        ...
        val zone = zone(plan.zoneId)
        val completedOn = plan.completedAt?.let { Instant.ofEpochMilli(it).atZone(zone).toLocalDate().toString() }
        return snapshot.copy(
            ...
            scheduleRevisions = listOf(ScheduleRevision(1, 1, plan.trackingStartedAt, "Imported fixed schedule")),
            ...
            targetHistory =
                listOf(
                    TargetHistoryEntry(
                        id = 1,
                        effectiveFrom =
                            Instant.ofEpochMilli(plan.trackingStartedAt).atZone(zone).toLocalDate().toString(),
    ...
    fun intervals(intervals: List<TrayIntervalDraft>, totalTrays: Int) {
        ...
        intervals.sortedBy { it.firstTray }.forEach {
            require(it.firstTray == next) { "Schedule intervals must have no gaps or overlaps." }
            require(it.lastTray in it.firstTray..totalTrays) { "Schedule interval is outside the phase." }
            require(it.daysPerTray in 1..365) { "Tray interval must be between 1 and 365 days." }
            next = it.lastTray + 1
        }
    ...
    fun appointment(appointment: Appointment) {
        ...
        require(appointment.title.isNotBlank() && appointment.title.length <= 500) { "Appointment title is required." }
        require(appointment.note.length <= MAX_TEXT_LENGTH) { "Appointment note is too long." }
        appointment.reminderMinutesBefore?.let { require(it in 0..525_600) { "Appointment reminder is invalid." } }
    }
    ...
    fun photo(photo: PhotoMetadata, now: Long, allowOwnership: Boolean) {
        ...
        require(photo.mimeType.matches(Regex("image/[A-Za-z0-9.+-]{1,64}"))) { "Photo MIME type is invalid." }
    ...
    private fun validateEvents(events: List<WearEvent>, plan: TreatmentPlan, now: Instant) {
        ...
        require(events.first().at == plan.trackingStartedAt) { "Initial event must match tracking start." }
        val ids = HashSet<Long>()
        var previous: WearEvent? = null
        for (event in events) {
            require(event.id > 0 && ids.add(event.id)) { "Event IDs must be positive and unique." }
            require(event.at in plan.trackingStartedAt..minOf(now.toEpochMilli(), plan.completedAt ?: Long.MAX_VALUE)) {
    ...
    private fun validateExpanded(snapshot: TrackerSnapshot, now: Instant) {
        ...
        require(active.size == if (plan.completed) 0 else 1) { "Treatment must have exactly one active phase." }
        var expectedOrdinal = 1
        snapshot.phases.sortedBy { it.ordinal }.forEach { phase ->
            require(phase.ordinal == expectedOrdinal++) { "Phase order must be contiguous." }
            require(phase.name.isNotBlank() && phase.name.length <= 200) { "Phase name is invalid." }
            require(phase.totalTrays in 1..1000) { "Phase tray count is invalid." }
            val started = date(phase.startedOn)
            phase.completedOn?.let { require(date(it) >= started) { "Phase completion precedes its start." } }
        }
        active.singleOrNull()?.let {
            require(it.totalTrays == plan.totalTrays) { "Active phase must match the treatment tray count." }
        }
        ...
        snapshot.scheduleRevisions.forEach {
            require(it.phaseId in phaseIds && it.createdAt in 0..now.toEpochMilli()) { "Schedule revision is invalid." }
            require(it.reason == null || it.reason.length <= 1000) { "Schedule reason is too long." }
            ...
            intervals(rows.map { row -> TrayIntervalDraft(row.firstTray, row.lastTray, row.daysPerTray) }, phase.totalTrays)
        }
        ...
        snapshot.trayHistory.forEach { tray ->
            val phase = snapshot.phases.singleOrNull { it.id == tray.phaseId }
            ...
            tray.endedOn?.let { require(date(it) >= started) { "Tray history ends before it starts." } }
            tray.startedAt?.let { require(it in plan.trackingStartedAt..now.toEpochMilli()) { "Tray history start time is invalid." } }
            tray.endedAt?.let { ended ->
                ...
                tray.startedAt?.let { require(ended >= it) { "Tray history end precedes its start." } }
            }
        }
        ...
        snapshot.targetHistory.sortedBy { it.effectiveFrom }.forEach {
            val effective = date(it.effectiveFrom)
            require(priorTargetDate == null || effective > priorTargetDate) { "Target dates must increase without duplicates." }
        ...
        snapshot.trackingGaps.sortedBy { it.startAt }.forEach { gap ->
            require(gap.startAt >= plan.trackingStartedAt && gap.startAt < gap.endAt) {
                "Tracking gap is outside treatment history."
            }
            require(gap.endAt <= minOf(now.toEpochMilli(), plan.completedAt ?: Long.MAX_VALUE)) {
                "Tracking gap is outside treatment history."
            }
            require(priorGapEnd == null || gap.startAt >= priorGapEnd!!) { "Tracking gaps cannot overlap." }
            priorGapEnd = gap.endAt
        }
        require(snapshot.notes.size + snapshot.appointments.size + snapshot.photos.size <= MAX_AUXILIARY_RECORDS) {
            "Too many treatment records."
        }
        positiveUnique(snapshot.notes.map { it.id }, "note")
        snapshot.notes.forEach {
            note(it, now.toEpochMilli())
            require(it.phaseId == null || it.phaseId in phaseIds) { "Note references a missing phase." }
            require(it.trayHistoryId == null || it.trayHistoryId in trayIds) { "Note references a missing tray." }
        }
        ...
        snapshot.photos.forEach {
            ...
            require(it.phaseId == null || it.phaseId in phaseIds) { "Photo references a missing phase." }
            require(it.trayHistoryId == null || it.trayHistoryId in trayIds) { "Photo references a missing tray." }
        }
    ...
    private fun hasExpandedTreatmentRecords(snapshot: TrackerSnapshot): Boolean =
        snapshot.phases.isNotEmpty() || snapshot.scheduleRevisions.isNotEmpty() ||
            snapshot.trayIntervals.isNotEmpty() || snapshot.trayHistory.isNotEmpty() ||
            snapshot.targetHistory.isNotEmpty() || snapshot.notes.isNotEmpty() ||
            snapshot.appointments.isNotEmpty() || snapshot.photos.isNotEmpty() ||
