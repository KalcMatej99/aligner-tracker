...
/** Reproducible synthetic Today-only emulator captures, not a human usability test. */
class TodayStatusCaptureTest {
    ...
    fun captureMatrix() {
        ...
        val now = Instant.parse("2026-09-10T16:30:00Z")
                ...
                MaterialTheme(
                    ...
                    Surface(Modifier.safeDrawingPadding().width(c.width.dp).fillMaxHeight()) {
                        ...
                        key(c.name, wearing) {
                            ...
                            TodayScreen(snapshot, now, c.busy, {})
                        }
                    }
                }
            }
        }
        for (c in cases) for (state in listOf(false, true)) {
            compose.runOnIdle {
                selected = c
                wearing = state
            }
            ...
            if (!c.completed) {
                compose
                    .onNodeWithText(
                        resources.getString(
                            if (c.busy) R.string.saving
                            else if (state) R.string.take_out else R.string.put_in
                        )
                    )
                    .assertIsDisplayed()
            ...
            if (
                !c.completed &&
                    InstrumentationRegistry.getArguments().getString("baseline") != "true"
            ) {
                compose
                    .onAllNodesWithContentDescription(
                        resources.getString(if (state) R.string.state_in else R.string.state_out)
                    )
                    .assertCountEquals(1)
                compose
    ...
    private fun capture(name: String) {
        ...
        val output =
            File(instrumentation.targetContext.getExternalFilesDir(null), "today-status/$name.png")
