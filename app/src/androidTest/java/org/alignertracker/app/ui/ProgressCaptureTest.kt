...
/** Reproducible synthetic Progress-only emulator captures, not a human usability test. */
class ProgressCaptureTest {
    ...
    fun captureMatrix() {
        ...
        val now = Instant.parse("2026-09-11T18:30:00Z")
        val base = progressFixture()
        val start = base.plan!!.trackingStartedAt
        data class Case(
            val name: String,
            val width: Int = 400,
            val font: Float = 1f,
                        ...
                        key(c.name, wearing) {
                            ...
                            val snapshot =
                                base.copy(
                                    ...
                                    events = if (c.name == "empty") emptyList() else base.events,
                                )
